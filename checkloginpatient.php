...
if(count($_GET)>0)
 {
	...
	$result = mysqli_query($conn,"SELECT * FROM Patient WHERE Name='" . $_GET["uname"] . "' and password = '". $_GET["psw"]."'");
    ...
    if($count > 0) //If there are no returning rows or no existing username
    {
       ...
       if(($name == $table_users) )// checks if there are any matching fields
       {
          if($password == $table_password)
          {
             ...
             header("location:patient_mode.php"); // redirects the user to the authenticated home page
          }
	else
         {
           Print '<script>alert("Incorrect Password or userame !");</script>'; // Prompts the user
    ...
    {
       Print '<script>alert("Incorrect username or passoword !");</script>'; // Prompts the user
        Print '<script>window.location.assign("login.php");</script>'; // redirects to login.php
    }
